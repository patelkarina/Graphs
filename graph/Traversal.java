...
public abstract class Traversal {
    ...
    protected Traversal(Graph G, Queue<Integer> fringe) {
        ...
        _postVertices = new ArrayList<Integer>();
    }
    ...
    public void traverse(Collection<Integer> V0) {
        ...
        while (!_fringe.isEmpty()) {
            int next = _fringe.remove();
            if (marked(next) && shouldPostVisit(next)
                    && !_postVertices.contains(next)) {
                postVisit(next);
                _postVertices.add(next);
            } else if (!marked(next)) {
                mark(next);
                visit(next);
                _fringe.add(next);
                for (int s : _G.successors(next)) {
                    if (processSuccessor(next, s)) {
                        _fringe.add(s);
                    }
                }
    ...
    private ArrayList<Integer> _postVertices;
