...
class Rule {
    ...
    private void checkFinishedDependencies() {
        int v = getVertex();
        for (int i : _depends.successors(v)) {
            Rule label = _depends.getLabel(i);
            if (label.isUnfinished()) {
                error("Error: it's time to rebuild!");
            }
        }
    }
