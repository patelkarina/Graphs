...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        if (segment.size() == 0 || segment.size() == 1) {
            return seq;
        }

        Road next = _map.getLabel(from, segment.get(1));
        String previousRoad = next.toString();
        Direction previousDirection = next.direction();
        double previousDistance = next.length();
        from = segment.get(1);
        int to = segment.get(1);

        String nextDescription = String.format("%d. Take %s %s for %.1f miles",
                seq, previousRoad,
                previousDirection.fullName(), previousDistance);

        for (int i = 2; i < segment.size(); i++) {
            to = segment.get(i);
            next = _map.getLabel(from, to);

            if (next.toString().equals(previousRoad)
                    && next.direction() == previousDirection) {
                previousDistance += next.length();
            } else {
                System.out.println(nextDescription + ".");
                seq++;

                previousRoad = next.toString();
                previousDirection = next.direction();
                previousDistance = next.length();
            }

            nextDescription = String.format("%d. Take %s %s for %.1f miles",
                    seq, previousRoad, previousDirection.fullName(),
                    previousDistance);
            from = segment.get(i);
        }

        nextDescription += String.format(" to %s",
                _map.getLabel(to).toString());
        System.out.println(nextDescription + ".");
        seq++;
        return seq;
